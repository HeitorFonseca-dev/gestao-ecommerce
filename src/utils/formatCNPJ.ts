export class ValidateCNPJUtils {
  private static _regexCNPJ = /^\d{2}.\d{3}.\d{3}\/\d{4}-\d{2}$/;

  public static validateCNPJ(cnpj: string | number | number[] = ''): boolean {
    if (!cnpj) {
      return false;
    }
  ...
  // Format CNPJ
  public static formatCNPJ(value: string | number | number[] = '') {
  ...
  // Calcule validator digit
  private static validCalc(x: number, numbers: number[]) {
  ...
  // Remove all not digit
  public static matchNumbers(value: string | number | number[] = '') {
